        ...
        ["Caitlyn"] = function(args)
            if Buff:HasBuff(args.From, "caitlynpassivedriver") then
                ...
                if args.TargetIsMinion then
                    ...
                    args.RawPhysical = args.RawPhysical + (1 + (modCrit * args.From.critChance)) * args.From.totalDamage * modLevel
                end
            end
        end,
        ["Corki"] = function(args)
...
Spell = {
    ...
    SpellClear = function(self, spell, spelldata, isReady, canLastHit, canLaneClear, getDamage)
        local hk
        if spell == _Q then hk = HK_Q elseif spell == _W then hk = HK_W
        elseif spell == _E then hk = HK_E elseif spell == _R then hk = HK_R end
        Health:AddSpell({
            HK = hk,
            spell = spell,
            isReady = isReady,
            ...
            Tick = function(self)
                ...
                local isLastHit = self.canLastHit() and (Orbwalker.Modes[ORBWALKER_MODE_LASTHIT] or Orbwalker.Modes[ORBWALKER_MODE_LANECLEAR])
                ...
                for i = 1, #targets do
                    ...
                    table_insert(self.FarmMinions, self:SetLastHitable(target, self.Delay + target.distance / self.Speed + Data:GetLatency(), self.getDamage()))
                end
